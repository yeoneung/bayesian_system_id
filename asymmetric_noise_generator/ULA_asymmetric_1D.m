...
writematrix(zeros(n,1)','asymmetric_noise_1D.csv')
for iteration = 1:step_iteration+1000000
    
    grad_U = k*x;

    if (alpha < x(n) <beta)
        grad_U = ((K-k)/(2*(beta-alpha))*(x(n))^2+(k-(K-k)*alpha/(beta-alpha))*x(n)+C_1);
    ...
    if iteration > step_iteration
        ...
        writematrix(x','asymmetric_noise_1D.csv','WriteMode','append')
        data{end+1} = {x};
    end
end
